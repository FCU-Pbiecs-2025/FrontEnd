<template>
  <div class="announcement-edit-page">
    <div class="announcement-edit-card">
      ...
      <div class="tab-row">
        <span class="tab-title">系統公告編輯</span>
      </div>
...
const goBack = () => {
  router.replace({ name: 'AdminAnnouncement' })
}
const save = () => {
  // 儲存功能可串接API或localStorage
  alert('儲存成功（可串接API）')
  router.replace({ name: 'AdminAnnouncement' })
}
</script>
...
<style scoped>
.announcement-edit-page { display:flex; justify-content:center; padding:32px 0; }
.announcement-edit-card { width:820px; background: #fff; border:1.5px solid #e6e6ea; border-radius:16px; padding:28px 32px; box-shadow:0 8px 24px rgba(16,24,40,0.04); }
